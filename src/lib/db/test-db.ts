...
async function testDatabaseOperations() {
  try {
    ...
    if (adminUser) {
      console.log(
        `✅ Admin user found: ${adminUser.username} (${adminUser.role})`
      );
    }
    ...
    console.log(
      `✅ Category hierarchy built with ${hierarchy.length} root nodes`
    );
    ...
    const productsWithVariants =
      await productService.findProductsWithVariants();
    console.log(
    ...
    console.log(
      `✅ Search for 'bhagavad' returned ${searchResults.products.length} results`
    );
    ...
    console.log(
      `✅ Full-text search for 'krishna' returned ${ftsResults.length} results`
    );
    ...
    if (
      ...
      productsWithVariants[0].variants.length > 0
    ) {
      ...
      console.log(
        `✅ Created transaction: ${transaction.id} (Total: ₹${transaction.total})`
      );

      // Test transaction retrieval
      const retrievedTransaction =
        await transactionService.findTransactionWithItems(transaction.id);
      console.log(
        `✅ Retrieved transaction with ${retrievedTransaction?.items.length} items`
      );
    }
    ...
    console.log(
      `✅ Today's sales: ₹${dailySales.totalSales} (${dailySales.totalTransactions} transactions)`
    );

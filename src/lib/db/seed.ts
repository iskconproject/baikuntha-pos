import { userService, categoryService, productService } from '@/services/database';
...
async function seedDatabase() {
  try {
    ...
    const booksCategory = await categoryService.createCategory({
      ...
      keywords: JSON.stringify(['books', 'literature', 'spiritual', 'religious', 'reading', 'pustaka', 'kitab'])
    });
    
    const accessoriesCategory = await categoryService.createCategory({
      name: 'Accessories',
      description: 'Religious accessories and items',
      keywords: JSON.stringify(['accessories', 'items', 'religious', 'spiritual', 'upkaran', 'samagri'])
    });
    
    const giftsCategory = await categoryService.createCategory({
      name: 'Gifts',
      description: 'Gift items and souvenirs',
      keywords: JSON.stringify(['gifts', 'souvenirs', 'presents', 'uphar', 'tohfa'])
    });
    
    // Create subcategories for books
    const srilaBooks = await categoryService.createCategory({
      name: 'Srila Prabhupada Books',
      description: 'Books by His Divine Grace A.C. Bhaktivedanta Swami Prabhupada',
      parentId: booksCategory.id,
      keywords: JSON.stringify(['prabhupada', 'founder', 'acharya', 'bhaktivedanta', 'swami'])
    });
    ...
    const bhagavadGitaBooks = await categoryService.createCategory({
      ...
      parentId: booksCategory.id,
      keywords: JSON.stringify(['bhagavad', 'gita', 'krishna', 'arjuna', 'philosophy'])
    });
    ...
    const srimadBhagavatamBooks = await categoryService.createCategory({
      ...
      parentId: booksCategory.id,
      keywords: JSON.stringify(['srimad', 'bhagavatam', 'purana', 'krishna', 'stories'])
    });
    ...
    const japaAccessories = await categoryService.createCategory({
      ...
      keywords: JSON.stringify(['japa', 'mala', 'chanting', 'beads', 'bag', 'tulsi'])
    });
    ...
    const deityAccessories = await categoryService.createCategory({
      ...
      parentId: accessoriesCategory.id,
      keywords: JSON.stringify(['deity', 'worship', 'puja', 'altar', 'murti', 'archana'])
    });
    ...
    const bgAsItIs = await productService.createProduct({
      ...
      description: 'The complete edition with original Sanskrit verses, word-for-word meanings, translations, and elaborate purports by His Divine Grace A.C. Bhaktivedanta Swami Prabhupada',
      basePrice: 350.00,
      categoryId: bhagavadGitaBooks.id,
      keywords: JSON.stringify(['bhagavad', 'gita', 'as it is', 'prabhupada', 'krishna', 'philosophy', 'complete']),
    ...
    await productService.createVariant({
      ...
      price: 350.00,
      stockQuantity: 50,
      attributes: JSON.stringify({ binding: 'hardcover', language: 'english', size: 'regular' }),
    ...
    await productService.createVariant({
      ...
      price: 250.00,
      stockQuantity: 100,
      attributes: JSON.stringify({ binding: 'paperback', language: 'english', size: 'regular' }),
    ...
    await productService.createVariant({
      ...
      price: 300.00,
      stockQuantity: 30,
      attributes: JSON.stringify({ binding: 'paperback', language: 'hindi', size: 'regular' }),
    ...
    const sbSet = await productService.createProduct({
      ...
      description: 'Complete 12 Canto set of Srimad Bhagavatam with original Sanskrit verses, translations, and purports',
      basePrice: 4500.00,
      categoryId: srimadBhagavatamBooks.id,
      keywords: JSON.stringify(['srimad', 'bhagavatam', 'complete', 'set', '12', 'canto', 'purana']),
      metadata: JSON.stringify({
        author: 'A.C. Bhaktivedanta Swami Prabhupada',
        publisher: 'The Bhaktivedanta Book Trust',
        volumes: 12,
    ...
    await productService.createVariant({
      ...
      price: 4500.00,
      stockQuantity: 10,
      attributes: JSON.stringify({ binding: 'hardcover', volumes: 12, language: 'english' }),
    ...
    const japaMala = await productService.createProduct({
      ...
      description: 'Hand-crafted Tulsi wood japa mala with 108 beads for chanting the holy names',
      basePrice: 150.00,
      categoryId: japaAccessories.id,
      keywords: JSON.stringify(['tulsi', 'japa', 'mala', 'beads', 'chanting', '108', 'holy', 'names']),
      metadata: JSON.stringify({
        material: 'Tulsi wood',
        beadCount: 108,
        origin: 'Vrindavan',
    ...
    const japaBag = await productService.createProduct({
      name: 'Japa Mala Bag',
      description: 'Cotton japa bag with drawstring for carrying and protecting your japa mala',
      basePrice: 50.00,
      categoryId: japaAccessories.id,
      keywords: JSON.stringify(['japa', 'bag', 'cotton', 'drawstring', 'protection', 'carry']),
      metadata: JSON.stringify({
        material: 'Cotton',
        closure: 'Drawstring',
        washable: true

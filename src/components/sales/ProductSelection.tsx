...
export function ProductSelection({ className = '' }: ProductSelectionProps) {
  ...
  const quickCategories = [
    ...
    { id: '2', name: 'Accessories', icon: '📿', color: 'bg-purple-100 text-purple-700' },
    { id: '3', name: 'Incense', icon: '🕯️', color: 'bg-yellow-100 text-yellow-700' },
    { id: '4', name: 'Clothing', icon: '👕', color: 'bg-green-100 text-green-700' },
  ...
  const handleQuickAdd = async (product: ProductSearchResult) => {
    try {
      ...
      if (productData.variants && productData.variants.length > 0) {
        setProductVariants(prev => ({ ...prev, [product.id]: productData.variants || [] }));
  ...
  const handleVariantSelect = async (productId: string, variant: ProductVariant) => {
    try {
          ...
          <div className="max-h-96 overflow-y-auto">
            {searchResults.map((product) => (
              <div key={product.id} className="p-4 border-b border-gray-100 last:border-b-0">
                ...
                {showVariantSelector === product.id && productVariants[product.id] && (
                  <div className="mt-4">
                    <VariantSelector
                      variants={productVariants[product.id]}
                      onSelect={(variant) => handleVariantSelect(product.id, variant)}

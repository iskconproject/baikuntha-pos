...
import { syncService, type SyncStatus, type SyncResult } from '@/services/database/sync';
import { offlineQueue, type QueueStats, type QueuedOperation } from '@/services/database/offlineQueue';
...
export function SyncManager({ className = '' }: SyncManagerProps) {
  ...
  const handleClearQueue = () => {
    if (confirm('Are you sure you want to clear all queued operations? This cannot be undone.')) {
      offlineQueue.clear();
      setQueueOperations([]);
    }
  };
  ...
  const getStatusColor = () => {
    ...
    if (syncStatus.isSyncing || isManualSyncing) return 'bg-yellow-500 animate-pulse';
  ...
  const getStatusText = () => {
    ...
    if (queueStats.pendingOperations > 0) return `${queueStats.pendingOperations} Pending`;
    return 'Synced';
  };
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            ...
            <Button
              ...
              disabled={!connectionStatus?.isOnline || syncStatus.isSyncing || isManualSyncing}
            >
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="text-xs text-green-700">
                ...
                {connectionStatus?.effectiveType && (
                  <span className="ml-2">({connectionStatus.effectiveType})</span>
                )}
        ...
        {queueStats.totalOperations > 0 && (
          <div className="p-3 bg-blue-50 rounded-lg border border-blue-200">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium text-blue-800">Operation Queue</span>
              <Button
                variant="outline"
                size="xs"
            ...
            <div className="grid grid-cols-3 gap-2 text-xs">
              <div>
                ...
                <span className="font-medium">{queueStats.pendingOperations}</span>
              </div>
              <div>
                <span className="text-red-600">Failed: </span>
                <span className="font-medium">{queueStats.failedOperations}</span>
              </div>
              <div>
                <span className="text-gray-600">Total: </span>
                <span className="font-medium">{queueStats.totalOperations}</span>
              </div>
            </div>
              ...
              {queueOperations.map((operation) => (
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center space-x-2">
                      <Badge
                        variant={operation.retryCount >= operation.maxRetries ? 'error' : 'info'}
                      >
                ...
                >
                  ...
                  <div className="grid grid-cols-3 gap-2 text-xs mb-2">
                    <div>
                      ...
                      <span className="font-medium">{result.tablesProcessed}</span>
                    </div>
                    <div>
                      <span className="text-gray-600">Records: </span>
                      <span className="font-medium">{result.recordsSynced}</span>
                    </div>
                    <div>
